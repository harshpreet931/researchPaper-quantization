...
\begin{enumerate}[leftmargin=*,nosep]
    \item \textbf{INT8 is essentially lossless for the 2B and larger models}: Qwen3.5-2B, Qwen3.5-4B, and Phi-3 Mini differ from FP16 by just --0.04, --0.04, and --0.07 pp, respectively. Qwen3.5-0.8B is the only model with a noticeable INT8 shift (--1.04 pp).

    \item \textbf{INT4 introduces real but model-dependent degradation}: overall drops are --2.59 pp for Qwen3.5-0.8B, --7.90 pp for Qwen3.5-2B, --3.29 pp for Qwen3.5-4B, and --2.39 pp for Phi-3 Mini. The 4B-class models remain usable at INT4, but the loss is no longer negligible.

    \item \textbf{Category structure is stable under quantization}: Social Sciences is the strongest category for every configuration, while the largest INT4 degradation occurs for Qwen3.5-2B across all categories (6.95--9.41 pp). Qwen3.5-4B and Phi-3 Mini retain strong INT4 category-wise performance, especially in Social Sciences (79.10\%, 76.49\%) and Other (76.32\%, 72.56\%).

    \item \textbf{Accuracy still scales with model quality}: Within the Qwen3.5 family, FP16 accuracy rises from 46.31\%$\to$56.59\%$\to$73.11\%, and INT4 rises from 43.73\%$\to$48.68\%$\to$69.82\%. The benefits of a stronger base model largely survive quantization.
\end{enumerate}
...
\begin{enumerate}[leftmargin=*,nosep]
    \item \textbf{Quantization level}: INT4 provides the best throughput--memory trade-off for 4B-class models when a 2--3 pp MMLU drop is acceptable. For smaller models or accuracy-sensitive deployments, INT8 is the safer default.

    \item \textbf{Model selection}: Qwen3.5-4B offers the best absolute accuracy at every precision, while Phi-3 Mini INT4 delivers the best large-model throughput with a slightly smaller INT4 accuracy drop than Qwen3.5-4B.
\end{enumerate}
